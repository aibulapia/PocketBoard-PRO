...
1. ** 다크모드 적용 
2. **폭염대책 자동 작성** — 안전보고문에 체감온도(공사전=최초/공사중=12~14시) + 조치 자동 삽입, 미측정 시 경고팝업
3. **기상 정보 팝업** — 양정동 실황 기온·습도 + 기상특보/통보문, 2시간 자동 갱신
4. **카톡 인앱 우회 + PWA 설치 안내**
5. **관리자(마스터) 모드** — 서버 검증, 현장 등록소, 연락처 서버 보관
6. **엑셀 다운로드** — 일반=내 세션 / 마스터=전체 통합, **매일 06시 구글시트 자동백업**
7. **작업연장** — "22시연장" 표기, 17시 후 배지 자동 전환(종료/연장)
8. **TODAY 터치 토글** — 회색 이탤릭 취소, 06시 리셋
9. **n일째 표시** — 실제 작업일 카운트
10. **data.js 분리** — 명단 수정은 이 파일만
11. **06시 일일 체계** — 상태·연장·표시 자동 리셋 (심야작업 보호)
12. ## 안전보고문 5공장만 적용 / 카카오톡 복사 삭제
