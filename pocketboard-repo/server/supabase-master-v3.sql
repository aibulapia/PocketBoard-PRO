-- ############################################################
-- ⚠️⚠️ 경고 — 이 파일을 그대로 재실행하지 마세요 ⚠️⚠️
--
-- 이 파일은 v2.5 시점 버전이라 v2.10에서 추가된 2단계 권한 체계
-- (master_level / master_guard_admin)가 빠져 있습니다.
-- 그대로 실행하면 실제 서버의 2단계 권한이 덮어써져 망가집니다.
--   · 일반 관리자 코드로 로그인 불가
--   · 접속 현황(레벨2 전용) 등 권한 판정 오작동
--
-- 관리자 코드/토큰을 바꾸려면 → supabase-rotate-codes-v1.sql 사용
-- 아래 내용은 테이블 구조·연락처 참고용으로만 보세요.
-- ############################################################
